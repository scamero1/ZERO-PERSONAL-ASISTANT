...
class FileProcessor:
    ...
    def extract_text_from_json(file_bytes: bytes, filename: str) -> str:
        """Extrae y formatea contenido de archivos JSON"""
        try:
    ...
    def process_file(file_bytes: bytes, filename: str) -> Dict[str, Any]:
        ...
        try:
            if file_type == 'pdf':
                ...
                result['content'] = FileProcessor.extract_text_from_csv(file_bytes, filename)
            elif file_type == 'json':
                result['content'] = FileProcessor.extract_text_from_json(file_bytes, filename)
            elif file_type == 'text':
